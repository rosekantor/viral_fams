...
def getargs():
    ...
    parser.add_argument("-g", "--genomeidlistfile", help="genome ids")
    parser.add_argument("-f", "--famidlistfile", help="fam ids")
    parser.add_argument("-s", "--sequencedir", help="genome sequence dir")
    ...
    if args.genomeidlistfile is None:
        print ("must specify --{}\n".format('genomeidlistfile'))
        args_pass = False
    elif not os.path.exists(args.genomeidlistfile) or \
         not os.path.isfile(args.genomeidlistfile) or \
         not os.path.getsize(args.genomeidlistfile) > 0:
        print ("--{} {} must exist and not be empty\n".format('genomeidlistfile', args.genomeidlistfile))
        args_pass = False

    if args.famidlistfile is None:
        print ("must specify --{}\n".format('famidlistfile'))
        args_pass = False
    elif not os.path.exists(args.famidlistfile) or \
         not os.path.isfile(args.famidlistfile) or \
         not os.path.getsize(args.famidlistfile) > 0:
        print ("--{} {} must exist and not be empty\n".format('famidlistfile', args.famidlistfile))
        args_pass = False
    ...
    if args.sequencedir is None:
        print ("must specify --{}\n".format('sequencedir'))
        args_pass = False
    elif not os.path.exists(args.sequencedir) or \
         not os.path.isdir(args.sequencedir):
        print ("--{} {} must exist\n".format('sequencedir', args.sequencedir))
        args_pass = False
...
# get_fam_seq_path ()
#
def get_fam_seq_path (out_dir, fam_id):
    fam_out_dir = os.path.join (out_dir, fam_id)
    if not os.path.exists(fam_out_dir):
        os.makedirs (fam_out_dir)
    fam_seq_file = fam_id+'-sliced.faa'
    fam_seq_path = os.path.join (fam_out_dir, fam_seq_file)
    return fam_seq_path
...
# read_fam_genome_hit_range ()
#
def read_fam_genome_hit_range (fam_hits_file, fam_ids, genome_ids):
    fam_genome_hits = dict()
    fam_genome_hit_range = dict()
    fields = []
    #fi_lookup = dict()
    ...
    with open (fam_hits_file, 'r') as hmmer_h:
        for hmmer_line in hmmer_h:
            ...
            #if not valid_hmm_hit (hit['tlen'], hit['hmm_from'], hit['hmm_to']):
            #    continue
            ...
            if gene_id not in fam_genome_hits[fam_id][genome_id]:
                fam_genome_hits[fam_id][genome_id][gene_id] = []
            fam_genome_hits[fam_id][genome_id][gene_id].append(hit)



    # reduce multi hits to range
    for fam_id in fam_genome_hits.keys():
        fam_genome_hit_range[fam_id] = dict()
        for genome_id in fam_genome_hits[fam_id].keys():
            for gene_id in fam_genome_hits[fam_id][genome_id].keys():
                hit = fam_genome_hits[fam_id][genome_id][gene_id][0]
                hmm_tlen = int(hit['tlen'])
                hmm_from = int(hit['hmm_from'])
                env_from = int(hit['env_from'])
                hmm_to   = int(hit['hmm_to'])
                env_to   = int(hit['env_to'])
                if len (fam_genome_hits[fam_id][genome_id][gene_id]) > 1:
                    for hit in fam_genome_hits[fam_id][genome_id][gene_id][1:]:
            ...
            # validate coverage
            if not valid_hmm_hit (hmm_tlen, hmm_from, hmm_to):
                print ("{} to genome {} gene {} is too short.  coverage={}".format(fam_id, genome_id, gene_id, ((float(hmm_to)-float(hmm_from)+1)/float(hmm_tlen))), flush=True)
                continue

            if genome_id not in fam_genome_hit_range[fam_id]:
                fam_genome_hit_range[fam_id][genome_id] = dict()
            fam_genome_hit_range[fam_id][genome_id][gene_id] = dict()
            fam_genome_hit_range[fam_id][genome_id][gene_id]['env_from'] = env_from
            fam_genome_hit_range[fam_id][genome_id][gene_id]['env_to'] = env_to
            
    return fam_genome_hit_range            


# valid_hmm_hit()
#
def valid_hmm_hit (hmm_len, hmm_ali_from, hmm_ali_to):
    valid = False
...
#
def write_fam_hit_sequences (out_dir,
                             sequence_dir,
                             fam_genome_hit_range):
    # build output
    for fam_id in sorted(fam_genome_hit_range.keys()):
        print ("DOING {}".format(fam_id))
        outbuf = []
        
        for genome_id in sorted(fam_genome_hit_range[fam_id].keys()):
            ...
            # build sequence buf
            for gene_id in sorted(fam_genome_hit_range[fam_id][genome_id].keys()):
                #for hit in fam_genome_hits[fam_id][genome_id][gene_id]:
                hit = fam_genome_hit_range[fam_id][genome_id][gene_id]
                env_from = hit['env_from']
                env_to = hit['env_to']
                seq_slice = gene_seqs[gene_id][int(env_from)-1:int(env_to)]
                new_id = genome_id+'-f:'+gene_id+'-slice:'+str(env_from)+'-'+str(env_to)
                mod_header = headers[gene_id].replace(gene_id,new_id)
                outbuf.append(mod_header)
                outbuf.append(seq_slice)
                
        # write outfile
        fam_seq_path = get_fam_seq_path (out_dir, fam_id)
...
def main() -> int:
    ...
    # get fam ids
    fam_ids = read_fam_ids (args.famidlistfile)

    # get genome ids
    genome_ids = read_genome_ids (args.genomeidlistfile)

    # read fam hits to genomes and genes
    fam_genome_hit_range = read_fam_genome_hit_range (args.inputhmmhitsfile,
                                                      fam_ids,
                                                      genome_ids)
    
    # write sequences for each fam
    write_fam_hit_sequences (args.outdir,
                             args.sequencedir,
                             fam_genome_hit_range)
